...
function MeetTheTiles({ onBack }) {
  ...
  const Section = ({ title, color, children, note }) => (
    <div style={{ marginBottom: 20 }}>
      <div style={{ fontFamily:"'Bodoni Moda',serif", fontSize:14, fontWeight:600, color: color||C.cherry, marginBottom: 6, letterSpacing:1 }}>{title}</div>
      {children}
      {note && <div style={{ fontSize:10.5, color:C.light, marginTop:6, lineHeight:1.5, fontStyle:"italic" }}>{note}</div>}
    </div>
  );
  ...
  const Label = ({ children }) => (
    <span style={{ fontSize:10, color:C.mid, fontWeight:500, marginLeft:2, marginRight:6 }}>{children}</span>
  );
  ...
  const ZoomOverlay = () => {
    ...
    return (
      <div onClick={() => setZoom(null)} style={{ position:"fixed", inset:0, background:"rgba(48,32,64,0.6)", backdropFilter:"blur(4px)", zIndex:1000, display:"flex", alignItems:"center", justifyContent:"center", flexDirection:"column", animation:"fadeIn 0.2s ease" }}>
        <div style={{ animation:"zoomIn 0.25s ease", display:"flex", flexDirection:"column", alignItems:"center", gap:10 }} onClick={e => e.stopPropagation()}>
          <div style={{ transform:"scale(1.6)" }}>{zoom.tile}</div>
          <div style={{ marginTop:16, textAlign:"center" }}>
            <div style={{ fontFamily:"'Bodoni Moda',serif", fontSize:18, fontWeight:600, color:"#fff" }}>{zoom.name}</div>
            {zoom.desc && <div style={{ fontSize:12, color:"rgba(255,255,255,0.7)", marginTop:4, maxWidth:240, lineHeight:1.4 }}>{zoom.desc}</div>}
          </div>
          <div style={{ fontSize:10, color:"rgba(255,255,255,0.4)", marginTop:8 }}>Tap anywhere to close</div>
        </div>
      </div>
    );
  };
    ...
    <>
      ...
      <Cnt>
        ...
        <Section title="Dragons" color="#4A7FA8">
          ...
          <div onClick={() => setDragonNote(!dragonNote)} style={{ marginTop:10, background:"linear-gradient(135deg," + C.paleBlueLt + "," + C.paleBlue + ")", borderRadius: dragonNote?14:10, padding: dragonNote?"12px 14px":"9px 14px", border:"1px solid rgba(173,212,236,0.3)", cursor:"pointer", transition:"all 0.3s ease" }}>
            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
              <div style={{ display:"flex", alignItems:"center", gap:5 }}>
                <span style={{ fontSize:12, fontWeight:700, color:"#4A96B8" }}>!</span>
                <span style={{ fontSize:11, fontWeight:400, color:C.dark }}>A Note on Dragons</span>
              </div>
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke={C.mid} strokeWidth="2" strokeLinecap="round" style={{ transition:"transform 0.3s", transform:dragonNote?"rotate(180deg)":"rotate(0deg)" }}><polyline points="6 9 12 15 18 9"/></svg>
            </div>
            {dragonNote && <div style={{ fontSize:10.5, color:C.mid, lineHeight:1.5, marginTop:8 }}>
        ...
        <Section title="Flowers" color="#8B7355" note="8 unique decorative tiles that appear in specific hands. One copy each.">
          <div style={{ display:"flex", gap:6, marginBottom:6, flexWrap:"wrap" }}>{[1,2,3,4].map(n => <div key={n} onClick={zt(<MiniFlower n={n} big/>, "Flower " + n, "A unique flower tile. One copy in the set.")}><MiniFlower n={n}/></div>)}</div>
          <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>{[5,6,7,8].map(n => <div key={n} onClick={zt(<MiniFlower n={n} big/>, "Flower " + n, "A unique flower tile. One copy in the set.")}><MiniFlower n={n}/></div>)}</div>
        </Section>

        <Section title="Jokers" color="#B8A9C9" note="8 wild tiles that can substitute for other tiles, with restrictions. One copy each.">
          <div style={{ display:"flex", gap:6, marginBottom:6, flexWrap:"wrap" }}>{[1,2,3,4].map(n => <div key={n} onClick={zt(<MiniJoker n={n} big/>, "Joker " + n, "A wild tile — can substitute for suited or honor tiles in groups of 3 or more.")}><MiniJoker n={n}/></div>)}</div>
          <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>{[5,6,7,8].map(n => <div key={n} onClick={zt(<MiniJoker n={n} big/>, "Joker " + n, "A wild tile — can substitute for suited or honor tiles in groups of 3 or more.")}><MiniJoker n={n}/></div>)}</div>
        </Section>
